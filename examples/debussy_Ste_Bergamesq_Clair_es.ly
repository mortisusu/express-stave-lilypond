...
\parallelMusic #'(rhUpRed rhDownGreen lhUpBlue lhDownGrey)
{
  ...
  s8*9 |
  \tieNeutral
  \tupletUp \stemDown \tuplet 2/3 {b'8\rest \pp <f gf bf>--~} <f gf bf> <ef gf bf>-- <ef gf bf>--
  %{ %}  <ef gf bf>-- <df gf bf>-- <df gf bf>-- \stemNeutral \tupletNeutral |
  <ef ef,>2. s4. |
  ...
  \tupletUp  \tuplet 2/3 {b8\rest <f' gf bf>--~} <f gf bf> <gf bf ef>-- <f gf bf>--
  %{ %}  <ef gf bf>-- <f gf bf>-- <ef gf bf>-- \tupletNeutral |
  <ef ef,>2. s4. |
  ...
  \stemUp r8*9/6 \stemDown \tuplet 6/9 {gf8[ af c bf gf] } |
  << { <af af,>2. s4.}
  ...
  % TODO: The base note seems incorrect
  \stemNeutral
  ...
  s8*9 |
  \stemUp r8 <bf df gf>--_( <bf df gf>-- <bf df gf>-- <bf df gf>--
...
\parallelMusic #'(rhUpRed rhDownGreen lhUpBlue lhDownGrey)
{
  ...
  \newSpacingSection
  \override Score.SpacingSpanner.spacing-increment = #1.4
  ...
  df16\( af' df f \cu af df\)
  \once \override Beam.positions = #'( 4 . 5)
  \cl f,,\( c' f\cu af c f\) \cl
  \once \override Beam.positions = #'( 5 . 6.5)
  af,,\( ff' af\cu cf ff af\) \cl |
  ...
  s4. \slurUp\stemNeutral \cl 
  \once \override Beam.positions = #'( 11 . 9.5)
  g'8\( \cu\stemDown f df df8. bf\) |
  ...
  \stemNeutral
  \once \override Beam.positions = #'( 7 . 9)
  af16\(ef' gf bf\cu c gf'\)\cl 
  \once \override Beam.positions = #'( 6 . 8)
  gf,,\( df' gf bf\cu df gf\)\cl 
  f,,\(\<c' f af\! c8\) |
  ...
  af,16\( ef' gf bf\cu c gf'\)\cl 
  \once \override Beam.positions = #'( 5 . 8.5)
  gf,,\( df' gf bf\cu df gf\)\cl 
  f,,\(\< c' f af\! c8\) |
  af4. gf \stemUp f \stemDown |
  ...
  bf'2. af4.) |
  s8*9 |
  \once \override Beam.positions = #'( 11.5 . 13)
  \stemNeutral d16\( gf bf\cu bf gf' bf\)\cl 
  \once \override Beam.positions = #'( 12 . 13.5)
  %{%} ef,,\( gf bf\cu bf gf' bf\)\cl 
  \once \override Beam.positions = #'( 11 . 14)
  c,,\( e af\cu c e af\)\cl |
  d4. ef af, |
  ...
  \once \override Beam.positions = #'( 11 . 13)
  af,\( c f af\cu c f\)\cl
  \once \override Beam.positions = #'( 11 . 13)
  cf,\( ff af\cu cf ff af\)\cl |
  df,4. f af |
...
\parallelMusic #'(rhUpRed rhDownGreen lhUpBlue lhDownGrey)
{
  ...
  % \ottava #1
  \stemDown <cs e>8\( <b ds> <a cs> \tuplet 2/3{<gs b> <fs a> ~} <fs a> <e gs> <ds fs>\) |
  s8*9 \f \ottava #0 |
  fs,,16^( cs' e fs a cs e fs e cs a fs) e( fs e cs a fs) |
  \mergeDifferentlyHeadedOn \mergeDifferentlyDottedOn fs2. ~ fs4. |

  \stemUp <cs e>8\( <b ds> <a cs> <gs b> <fs a> <e gs>^~<e gs> <ds fs> <cs e> \) \stemNeutral |
  \stemDown \dimTextDim s2.\>a,,4.\! \stemNeutral |
  \clef bass \slurUp
  fs,16( cs' e fs a cs e cs a fs e cs) \slurNeutral fs,( cs' fs, cs' fs, cs') |
  s8*9 \bar "||"
...
\parallelMusic #'(rhUpRed rhDownGreen lhUpBlue lhDownGrey)
{
  \barNumberCheck#43
  \clef alto
  \tempo"Calmato" \key df \major \tieUp \stemUp
  <df ef>2.( <ef gf>4 <ef gf bf>8) |
	\stemDown \slurDown gf4(\pp af8 bf4 df8 \stemDown c4.) |
  \key df \major 
  ...
  \clef alto \stemUp df'2. \stemNeutral c16_\( ef gf bf \cu c ef\) \cl |
  gf4_( af8 bf4 df8 c4.) |
...
\parallelMusic #'(rhUpRed rhDownGreen lhUpBlue lhDownGrey)
{
  ...
  r8 s4 \stemNeutral <f' af>4. <df f>\) ~ |
  s8*9 |
  % \stemNeutral\tieUp \once\override Beam.positions = #'(3 . 3.5)
  \stemNeutral \cl <df, af'>8 [ <f' af> <f' af>]~<f af cf,_> >2. |
  s4. s2. |

  \stemUp <df f>8( ef f ef2.) ~ |
  r8 <gf, bf>4~<gf bf>2. |
  \stemUp r8 <df, gf bf df>4 ~ <df gf bf df>2. |
  <gf, df'>2. s4. |
...
\parallelMusic #'(rhUpRed rhDownGreen lhUpBlue lhDownGrey)
{
  ...
  df16\( af' df f\cu af df\)
  \once \override Beam.positions = #'( 4 . 7)
  \cl f,,\( c' f\cu af c f\)
  \once \override Beam.positions = #'( 4 . 7)
  \cl df,,\( af' df f\cu af df\)\cl |
  df4. f df |
  ...
  f,,16\( c'f \cu af c f\)\cl\stemUp 
  \once \override Beam.positions = #'( 6 . 7)
  \cl af,,\( ff'  af \stemDown \cu cf ff gf \shiftOff af4.\) \cl |
  f4. af2. |

  r4 s8 r4 s8 r4 s8 |
  s8*9 |
  \stemNeutral
  \once \override Beam.positions = #'( 9 . 13)
  df,,16\( af' df f\cu af df\)
  \once \override Beam.positions = #'( 9 . 13)
  \cl f,,\( c' f\cu af c f\)
  \once \override Beam.positions = #'( 9 . 13)
  ...
  r4 s8 cf'4.( df) |
  s4 s8 af'2. |
  \once \override Beam.positions = #'( 10 . 14)
  \stemUp f,,16\( c' f\cu \stemDown af c f\)
  \once \override Beam.positions = #'( 10 . 14)
  \voiceTwo \cl \stemUp af,, \(cf ff af\cu \stemDown cf ff~  ff4.\)\cl |
  f4. af2. |
  ...
  af2. s4. |
  \once \override Beam.positions = #'( 11 . 13)
  \stemUp af,,16\( cf ff af\cu \stemDown cf ff~ff4.\) \cl \stemUp  af,,,4. \stemNeutral |
  af2. s4. |

  s8*9 |
  \tieNeutral <f' af>2. ~ <f af>4. |
  \voiceTwo \stemUp df,16\( af' df f! af df\cu f8 af df f af df\) \cl |
  df,,2. s4. |
